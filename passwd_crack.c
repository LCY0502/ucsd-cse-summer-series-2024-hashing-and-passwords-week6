...
int crack_at(
    ...
    //end of loop body
        ...
        loop[index] += 1;
    //a loop overflow
        while(loop[index] == MAX)
        {
            if(index == potential_len - 1)
            {
                return 0;
            }
...
void hard(unsigned char* user_input)    //assume input is the hashed value in hexdecimal 
{
    ...
    {   
        ...
        unsigned int result = crack_at(&attempts,&this_attempts,potential_len,potential_passwd,hash,tocheck,user_input,loop);
        printf("\tFor n = %d\tIt took %ld attempts\n", potential_len,  this_attempts);
        if(result) return;
        // printf("\n\n");
        potential_len += 1;
